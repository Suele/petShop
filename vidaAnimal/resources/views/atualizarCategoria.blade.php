...
<html lang="en">
  <head>
    ...
    <title>Vida Animal - Atualizar Categoria</title>
  </head>
  <body>
        ...
        <div class="container mt-5">
                @if(session('success'))
                 {{ session('success') }}
                @endif
                <h2 class="h5 text-muted mt-2 pb-2">Atualizar Categoria</h2>
                {!! Form::open(['action' => ['CategoryController@update', $category->id], 'method' => 'POST']) !!}
                         <div class="col-4 mt-4">
                             {{ Form::label('code', 'Código da Categoria') }}
                             {{ Form::text('code', $category->code, ['class' => 'form-control', 'placeholder' => 'Código da categoria', 'Required']) }}
                         </div>
                         <div class="col-6 mt-4">
                             {{ Form::label('name_category', 'Nome da Categoria') }}
                             {{ Form::text('name_category', $category->name_category, ['class' => 'form-control', 'placeholder' => 'Nome da Categoria', 'Required']) }}
                         </div>
                         {{ Form::hidden('_method', 'PUT') }}
                         {{ Form::submit('Atualizar', ['class' => 'btn btn-outline-success btn-lg mt-3 mt-4']) }}
